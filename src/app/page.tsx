...
export default async function Home() {
  ...
  const golfersIdMap = golfers.reduce((acc, player) => ({
    ...acc,
    [player.id]: player,
  }), {});
  ...
  return (
    <div className="p-4">
      ...
      {picks
        .sort((a_pick, b_pick) => (
          Object.values(a_pick.picks).reduce((acc, id) => acc + scoreToInt(golfersIdMap[id].statistics.find((stat) => stat.name === 'scoreToPar').displayValue), 0) -  Object.values(b_pick.picks).reduce((acc, id) => acc + scoreToInt(golfersIdMap[id].statistics.find((stat) => stat.name === 'scoreToPar').displayValue), 0)
        ))
        .map((pick, i) => (
        <div className="mb-8">
          <div className='flex gap-4 p-4 bg-emerald-400 text-black font-bold rounded-t-2xl'>
            ...
            <div className="shrink-0 grow-0">Overall: {parseScore(Object.values(pick.picks).reduce((acc, id) => acc + scoreToInt(golfersIdMap[id].statistics.find((stat) => stat.name === 'scoreToPar').displayValue), 0))}</div>
          </div>
          ...
          <div className="grid grid-cols-[1fr_50%_1fr_1fr] text-center gap-0.5 bg-emerald-950 [&>div]:bg-white  rounded-b-2xl border-2 border-emerald-950 overflow-hidden">
              {Object
                .values(pick.picks)
                .sort((a_id, b_id) => scoreToInt(golfersIdMap[a_id].statistics.find((stat) => stat.name === 'scoreToPar').displayValue) - scoreToInt(golfersIdMap[b_id].statistics.find((stat) => stat.name === 'scoreToPar').displayValue))
                .map((id) => (
                  <>
                    ...
                    <div className="p-2 text-ellipsis overflow-hidden grid items-center justify-center">{golfersIdMap[id].statistics.find((stat) => stat.name === 'scoreToPar').displayValue}</div>
                  </>
              ))}
          </div>
        </div>
