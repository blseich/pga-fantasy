...
export default function Rankings({ users }: { users: UserWithPickDetails[] }) {
  ...
  const findMe = useCallback(() => {
    ...
    refsArray.current[meIndex]?.scrollIntoView({
      behavior: 'smooth',
    });
    refsArray.current[meIndex]?.classList.add('highlighted');
    setTimeout(
      () => refsArray.current[meIndex]?.classList.remove('highlighted'),
      2000,
    );
  }, [refsArray, users]);
    ...
    <div className="flex w-full flex-col items-center gap-4">
      {users.map((user, i) => (
        ...
        >
          ...
          <div
            className={`${activeCards.includes(i) ? 'max-h-80' : 'max-h-0'} overflow-hidden bg-black transition-all duration-500 ease-out`}
          >
