...
type UserWithPicks = Pick<
  Tables<'profiles'>,
  'user_id' | 'first_name' | 'last_name' | 'public_id'
> & { picks: Tables<'picks'>[] } & {
  tiebreakers: { tiebreaker_score: number }[] | null;
};
...
const generateRankings = (
  ...
  const rankings = users.map((user) => {
    const picksWithScores = user.picks
      .map((pick) => {
        const golfer =
        ...
        return {
          ...
          score: {
            today:
              golfer.scoringData.position === 'CUT'
                ? 'CUT'
                : golfer.scoringData.thru === ''
                  ? golfer.scoringData.teeTime || ''
    ...
    return {
      ...
      score: picksWithScores.slice(0, 3).reduce(
        (acc, pick) => {
...
const getTargetDate = (date: string) => {
  const [month, firstDay, _dash, _secondDay, year] = date.split(' ');
  ...
  return (
    <>
      ...
      {tournament.tournamentStatus === 'NOT_STARTED' ? (
        <Countdown
          targetDate={getTargetDate(tournament.displayDate)}
          pickLink={`/user/${users?.find((u) => u.user_id === user?.id)?.public_id}`}
        />
      ) : (
