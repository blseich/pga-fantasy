    ...
    return (
        <div className="flex flex-col items-center justify-center">
            {picks.map((pick) => {
                ...
                const pickable = Boolean(golfer);

                return (
                    <div className="flex items-center gap-2 border-b-2 border-b-gray-500 w-11/12 mb-4 px-2 pb-4" key={pick.player_name}>
                        {pickable ? <GolferImage src={golfer.headshot?.href} alt={`${first} ${last}`} /> : <X className="w-[75px] h-[75px]"/>}
                        <div>
                        ...
                        {pickable ? <PickButton golfer_id={golfer.id} bucket={bucket} /> :  (
                            <div className="ml-auto text-red-500 aspect-square h-[40px] grid place-content-center font-bold">
                                OUT
                            </div>
                        )}
                    </div>
